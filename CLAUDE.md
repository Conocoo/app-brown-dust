...
**main 브랜치에 직접 커밋/push 금지. 반드시 feature 브랜치 → PR → merge 흐름을 따를 것.**

커밋이 필요한 작업 시:
1. `git checkout -b <접두사>/<작업명>` 으로 브랜치 생성
2. 작업 완료 후 커밋 & `git push -u origin <브랜치명>`
3. PR 생성 → 사용자에게 확인 요청 → merge

브랜치 접두사:
- `feature/` — 새 기능 (예: `feature/arines-skills`)
- `fix/` — 버그 수정 (예: `fix/crit-damage-calc`)
- `docs/` — 문서만 변경 (예: `docs/skill-list-update`)
- `refactor/` — 리팩토링 (예: `refactor/turn-logic`)
